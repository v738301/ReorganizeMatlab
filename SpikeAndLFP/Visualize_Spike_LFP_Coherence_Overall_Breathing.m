%% ========================================================================
%  VISUALIZE SPIKE-BREATHING COHERENCE (OVERALL)
%  Individual session viewer for overall breathing coherence results
%  ========================================================================
%
%  This script visualizes spike-breathing coherence for a single session.
...
fprintf('=== VISUALIZING SPIKE-BREATHING COHERENCE (OVERALL) ===\n');
...
result_files = dir(fullfile(data_path, '*_spike_breathing_coherence_overall.mat'));
...
fprintf('  Breathing channel: %d\n\n', session_results.breathing_channel);
...
if ~isempty(freq)
    ...
    % Plot heatmap
    figure('Position', [50, 50, 1400, 800], 'Name', 'Spike-Breathing Coherence Spectra Heatmap');
    ...
    title(sprintf('Spike-Breathing Coherence Spectra (Ch 32) - %s', session_results.filename), ...
        'FontSize', 13, 'FontWeight', 'bold', 'Interpreter', 'none');

    % Add vertical lines for breathing-relevant frequency bands
    hold on;
    % Breathing: 1-4 Hz, Sniffing: 4-12 Hz
    ...
    xline(2, 'r--', 'LineWidth', 2, 'Alpha', 0.5);
    text(2.5, n_units*0.95, '2 Hz (breathing)', 'Color', 'white', 'FontSize', 10, 'FontWeight', 'bold');
...
figure('Position', [100, 100, 1600, 1000], 'Name', 'Individual Unit Breathing Coherence Spectra');
...
for plot_idx = 1:length(top_units)
    ...
    % Highlight 2 Hz (typical breathing)
...
sgtitle(sprintf('Top 16 Units by Mean Breathing Coherence - %s', session_type_name), ...
...
% Plot
figure('Position', [150, 150, 1200, 600], 'Name', 'Band-Specific Breathing Coherence');
...
xtickangle(45);
ylabel('Mean Coherence', 'FontSize', 11);
title('Population Mean Breathing Coherence by Band', 'FontSize', 12, 'FontWeight', 'bold');
grid on;
set(gca, 'FontSize', 10);

sgtitle(sprintf('Frequency Band Analysis - Breathing (Ch 32) - %s', session_results.filename), ...
    'FontSize', 13, 'FontWeight', 'bold', 'Interpreter', 'none');
...
fprintf('\n=== SUMMARY STATISTICS (BREATHING COHERENCE) ===\n');
fprintf('Session: %s\n', session_results.filename);
...
fprintf('\nBand-specific mean breathing coherence:\n');
for b = 1:n_bands
    fprintf('  %s: %.3f ± %.3f\n', band_names{b}, mean_band_coherence(b), sem_band_coherence(b));
end

fprintf('\nBreathing-relevant bands:\n');
fprintf('  Delta (1-4 Hz, breathing): %.3f\n', mean_band_coherence(1));
fprintf('  Theta (5-12 Hz, sniffing): %.3f\n', mean_band_coherence(2));
...
fprintf('  - High coherence at 5-12 Hz → Coupling to sniffing/exploratory breathing\n');
fprintf('  - Compare with LFP coherence to identify breathing-specific vs general rhythmic coupling\n');
fprintf('========================================\n');
