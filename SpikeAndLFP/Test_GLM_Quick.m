%% ========================================================================
%  QUICK GLM TEST: Test on a few units with simple visualization
%  ========================================================================
%
%  This script tests the GLM analysis on a small subset of data:
%  - Loads 2-3 sessions
%  - Processes 2-3 units per session
%  - Fits GLM and creates simple visualizations
%  - Verifies model works before running full analysis
%% ========================================================================

clear all
close all

fprintf('=== QUICK GLM TEST ===\n\n');
...
% Interaction terms
config.include_interactions = true;  % Include interaction terms
config.interactions = {'Speed', 'Breathing'};  % Speed × Breathing interaction
...
feature_names = {'IR1ON', 'IR2ON','Aversive', 'Speed', 'Breathing'};
if config.include_interactions
    feature_names{end+1} = 'Speed_x_Breathing';
end
if config.include_history
...
% Build design matrix with raised cosine basis for events and continuous predictors
    ...
    % Create raised cosine log-stretched basis functions spanning [-1, +2] seconds
    n_basis = config.n_basis_funcs;
    ...
    if ~isempty(behavioral_matrix_full) && size(behavioral_matrix_full, 2) >= 8
        ...
        breathing_binned = binContinuousSignal(breathing_rate_raw, NeuralTime, time_centers);
        breathing_smoothed = smoothdata(breathing_binned, 'gaussian', round(config.smooth_window/config.bin_size));
        breathing_normalized = zscore(breathing_smoothed);

        predictor_matrix = [predictor_matrix, breathing_normalized];
        predictor_names{end+1} = 'Breathing';
    else
        % If breathing not available, add zeros
        predictor_matrix = [predictor_matrix, zeros(n_bins, 1)];
        predictor_names{end+1} = 'Breathing';
