%% ========================================================================
%  SPIKE-BREATHING COHERENCE ANALYSIS (OVERALL)
%  Computes coherence between spikes and breathing signal for all units
%  Uses multitaper method for robust spectral estimation
%  ========================================================================
%
%  This script:
%  1. Loads spike and breathing data for all sessions
%  2. For each unit, computes spike-breathing coherence across frequencies (1-150 Hz)
...
fprintf('=== SPIKE-BREATHING COHERENCE ANALYSIS (OVERALL) ===\n');
...
config.bp_range = [1, 300];          % Bandpass filter range for raw breathing
...
fprintf('  Breathing bandpass: %.1f-%.1f Hz\n\n', config.bp_range(1), config.bp_range(2));
...
for spike_sess_idx = 1:num_aversive_sessions
    ...
    fprintf('  Breathing channel: %d, Duration: %.1f min, Units: %d\n', ...
        config.breathing_channel, (NeuralTime(end) - NeuralTime(1))/60, length(valid_spikes));
    ...
    for unit_idx = 1:n_units
        ...
        % Compute coherence
        try
            [coherence, phase, freq, S_spike, S_breathing] = ...
                calculate_spike_lfp_coherence_multitaper(spike_times, Breathing, NeuralTime, Fs, coherence_params);
            ...
            unit_result.S_breathing = S_breathing;
    ...
    save_filename = fullfile(RewardAversivePath, sprintf('%s_spike_breathing_coherence_overall.mat', base_filename));
...
for spike_sess_idx = 1:num_reward_sessions
    ...
    [~, base_filename, ~] = fileparts(spike_filename);
    save_filename = fullfile(RewardSeekingPath, sprintf('%s_spike_breathing_coherence_overall.mat', base_filename));
    save(save_filename, 'session_results', 'config', '-v7.3');
