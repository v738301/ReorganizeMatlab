function [model1, model2, model3] = fitNestedModels_robust_3models(spike_counts, DM1, DM2, DM3, config)
% Fit 3 nested Poisson GLMs using robust optimization with analytical gradients
% Adapted from Pillow Lab's GLMspiketools
%
% NESTED MODEL STRUCTURE (3 MODELS):
%   Model 1: Intercept + Spike History + DM1 (baseline - all predictors except speed & breathing)
%   Model 2: Intercept + Spike History + DM2 (DM1 + Speed)
%   Model 3: Intercept + Spike History + DM3 (DM2 + Breathing)
%
% RATIONALE:
%   Start with a baseline model including intrinsic dynamics (spike history),
%   task-related events, spatial encoding, and kinematics. Then sequentially test
%   whether locomotion (speed) and respiratory modulation (breathing) add explanatory power.
%
...
%   DM1: [n_bins × n_predictors_1] Design matrix 1 (Events + Coords + Spatial)
%   DM2: [n_bins × n_predictors_2] Design matrix 2 (DM1 + Speed)
%   DM3: [n_bins × n_predictors_3] Design matrix 3 (DM2 + Breathing)
...
%
% Outputs:
%   model1, model2, model3: Model structs with:
...
%     - LRT_vs_previous: Likelihood ratio test statistic
%     - LRT_df: Degrees of freedom for LRT
%     - LRT_p_value: P-value for LRT
...
%% Fit Model 1: Baseline (Events + Coords + Spatial)
fprintf('\n--- Fitting Model 1: Baseline (Events + Coords + Spatial) ---\n');

% Design matrix: intercept + spike history + DM1
% DM1 contains: Events + Coordinates + Spatial predictors
DM_model1 = [ones(n_bins, 1), history_matrix, DM1];
...
% Likelihood Ratio Test vs Null model (constant firing rate)
model1.LRT_vs_previous = -2 * (null_ll - model1.log_likelihood);
model1.LRT_df = model1.n_predictors;  % Null model has 0 predictors
model1.LRT_p_value = 1 - chi2cdf(model1.LRT_vs_previous, model1.LRT_df);
...
%% Fit Model 2: Baseline + Speed
fprintf('\n--- Fitting Model 2: + Speed ---\n');

% Design matrix: intercept + spike history + DM2
% DM2 contains: Events + Coordinates + Spatial + Speed predictors
DM_model2 = [ones(n_bins, 1), history_matrix, DM2];
...
%% Fit Model 3: Full model (+ Breathing)
fprintf('\n--- Fitting Model 3: Full model (+ Breathing) ---\n');

% Design matrix: intercept + spike history + DM3
% DM3 contains: Events + Coordinates + Spatial + Speed + Breathing predictors
DM_model3 = [ones(n_bins, 1), history_matrix, DM3];
...
%% Summary
fprintf('\n✓ All 3 nested models fitted successfully\n');
fprintf('\n=== NESTED MODEL COMPARISON ===\n');
fprintf('%-25s %8s %8s %8s %10s %8s\n', 'Model', 'Dev.Exp', 'AIC', 'BIC', 'LRT (χ²)', 'p-value');
fprintf('%s\n', repmat('-', 1, 80));
fprintf('%-25s %7.2f%% %8.1f %8.1f %10.2f %8.4f %s (vs null)\n', ...
    '1. Baseline', model1.deviance_explained, model1.AIC, model1.BIC, ...
    model1.LRT_vs_previous, model1.LRT_p_value, ...
    getSignificanceStr(model1.LRT_p_value));
fprintf('%-25s %7.2f%% %8.1f %8.1f %10.2f %8.4f %s\n', ...
    '2. + Speed', model2.deviance_explained, model2.AIC, model2.BIC, ...
    model2.LRT_vs_previous, model2.LRT_p_value, ...
    getSignificanceStr(model2.LRT_p_value));
fprintf('%-25s %7.2f%% %8.1f %8.1f %10.2f %8.4f %s\n', ...
    '3. + Breathing', model3.deviance_explained, model3.AIC, model3.BIC, ...
    model3.LRT_vs_previous, model3.LRT_p_value, ...
    getSignificanceStr(model3.LRT_p_value));
fprintf('%s\n', repmat('=', 1, 80));
fprintf('\nModel Selection:\n');
[~, aic_best] = min([model1.AIC, model2.AIC, model3.AIC]);
[~, bic_best] = min([model1.BIC, model2.BIC, model3.BIC]);
