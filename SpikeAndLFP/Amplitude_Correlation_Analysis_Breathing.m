%% ========================================================================
%  AMPLITUDE CORRELATION ANALYSIS WITH BREATHING SIGNAL
%  Spike rate vs Breathing amplitude: Linear & Nonlinear coupling
%  ========================================================================
%
%  Purpose: Test for amplitude modulation with breathing signal
%
%  Method: For each frequency band (0.1-20 Hz):
%  1. Extract breathing amplitude envelope (Hilbert transform)
...
fprintf('=== AMPLITUDE CORRELATION ANALYSIS WITH BREATHING ===\n');
...
for sess_idx = 1:num_aversive_sessions
    ...
    session_results = compute_amplitude_correlation(NeuralTime, Breathing, valid_spikes, period_boundaries, ...
    ...
    save_filename = fullfile(RewardAversivePath, sprintf('%s_ampcorr_breathing.mat', base_filename));
...
for sess_idx = 1:num_reward_sessions
    ...
    save_filename = fullfile(RewardSeekingPath, sprintf('%s_ampcorr_breathing.mat', base_filename));
    save(save_filename, 'session_results', 'config', '-v7.3');
...
function session_results = compute_amplitude_correlation(NeuralTime, Breathing, valid_spikes, ...
...
% OPTIMIZED VERSION: Pre-allocated arrays, vectorized operations
    ...
    for band_idx = 1:n_bands
        ...
        % Filter breathing signal and extract amplitude envelope
        Breathing_filtered = filter_breathing_for_amplitude(Breathing, band_range, Fs);
        amplitude_envelope = abs(hilbert(Breathing_filtered));
...
function Breathing_filtered = filter_breathing_for_amplitude(Breathing, band_range, Fs)
% Filter breathing signal for amplitude extraction (same as PPC)
