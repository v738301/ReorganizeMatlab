...
config.event_window_pre = 1.0;          % Window for convolution padding (1s before)
...
event_names = {'IR1ON', 'IR2ON', 'Aversive', 'HighBreathingStart'};

% Create basis functions (for reconstruction)
basis_funcs = createGaussianBasis(config.basis_centers, config.basis_width, event_duration_bins, config.bin_size);
...
feature_names = {'IR1ON', 'IR2ON', 'Aversive', 'HighBreathingStart', 'Speed'};
if config.include_history
...
example_unit_idx = 8; % Use the same example unit
unit = all_glm_results(example_unit_idx);
...
plot_groups_names = {'Bias', event_names_gl{1}, 'Speed', 'Breathing'};
if config.include_interactions
...
function [dm, predictor_info] = buildDesignMatrixForSession(...
    NeuralTime, IR1ON, IR2ON, AversiveSound, ...
    AdjustedXYZ_speed, behavioral_matrix_full, config)
% Build design matrix with raised cosine basis for events and continuous predictors
    ...
    %% 1. EVENT PREDICTORS with Gaussian basis

    % Create HighBreathingStart event signal
    HighBreathingStart = zeros(size(NeuralTime));
    if ~isempty(behavioral_matrix_full) && size(behavioral_matrix_full, 2) >= 8
        breathing_rate_raw = behavioral_matrix_full(:, 8);
        % Detect onsets of high breathing (threshold crossing from below to above 5Hz)
        high_breathing = breathing_rate_raw > config.breathing_freq_threshold;
        high_breathing_onsets = find(diff([0; high_breathing(:)]) == 1);
        if ~isempty(high_breathing_onsets)
            HighBreathingStart(high_breathing_onsets) = 1;
        end
    end

    % Define event signals and names
    event_signals = {IR1ON, IR2ON, AversiveSound, HighBreathingStart};
    event_names = {'IR1ON', 'IR2ON', 'Aversive', 'HighBreathingStart'};
    ...
    for ev = 1:length(event_signals)
        ...
        if ~isempty(event_onset_indices)
            ...
            event_counts = histcounts(event_times, time_bins)';
    ...
    % NOTE: Breathing is now treated as EVENTS (HighBreathingStart), not continuous
    % No Speed × Breathing interaction in this version
