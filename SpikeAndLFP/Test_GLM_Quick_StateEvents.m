%% ========================================================================
%  QUICK GLM TEST: BREATHING EVENTS VERSION
%  ========================================================================
%
%  This version treats high breathing rate as discrete EVENTS:
%  - Creates "HighBreathingStart" event when breathing crosses >5Hz threshold
%  - Removes continuous breathing rate predictor
%  - Removes Speed × Breathing interaction
%
%  Predictors:
%  - Events: IR1ON, IR2ON, Aversive, HighBreathingStart (NEW!)
%  - Continuous: Speed (only)
...
fprintf('=== QUICK GLM TEST: BREATHING EVENTS VERSION ===\n\n');
...
config.history_lags = 5;           % 1-5 lags (50ms bins = 0-0.25 seconds)

% Interaction terms (DISABLED for breathing events version)
config.include_interactions = false;  % No interactions in this version
...
config.breathing_freq_threshold = 5;  % Threshold for high breathing event (Hz)
config.speed_threshold = 100;         % Threshold for high speed event
config.breathing_event_mode = true;   % Treat breathing as events, not continuous
...
for sess_idx = 1:n_sessions
    ...
    [dm, predictor_info] = buildDesignMatrixForSession(...
        ...
        AdjustedXYZ_speed, behavioral_matrix_full, config);
...
event_names = {'IR1ON', 'IR2ON', 'HighBreathingOn', 'HighBreathingOff', 'HighSpeedOn', 'HighSpeedOff'};
...
feature_names = {'IR1ON', 'IR2ON', 'HighBreathingOn', 'HighBreathingOff', 'HighSpeedOn', 'HighSpeedOff'};
if config.include_history
...
% Order: Intercept, IR1ON, IR2ON, Aversive, Speed, Breathing, Interaction, History
predictor_groups_to_add = {'Bias', event_names_gl{:}, 'Speed', 'Breathing'};
if config.include_interactions
    predictor_groups_to_add{end+1} = 'Speed_x_Breathing';
end
if config.include_history
...
term_colors = [0 0 1;    % Blue for Intercept
               ...
               0.5 0.5 0.5;% Gray for Breathing
...
for i = 1:length(predictor_groups_to_add)
    ...
    else % Continuous predictors ('Speed', 'Breathing', 'Speed_x_Breathing')
...
plot_groups_names = {'Bias', event_names_gl{:}};
if config.include_interactions
    plot_groups_names{end+1} = 'Speed_x_Breathing';
end
if config.include_history
    plot_groups_names{end+1} = 'History';  % Will plot mean of all lags
end

plot_colors = {'k', 'r', 'b', 'g', 'm', 'c','k','r'}; % Colors for the selected predictors
plot_line_styles = {'-', '--', ':', '-.', '-', '--',':','-.'};
...
function [dm, predictor_info] = buildDesignMatrixForSession(...
    NeuralTime, IR1ON, IR2ON, AversiveSound, ...
    AdjustedXYZ_speed, behavioral_matrix_full, config)
% Build design matrix with state on/off events for speed and breathing
    ...
    %% 1. EVENT PREDICTORS with basis functions

    % --- Create state-based On/Off events ---
    
    % High Breathing On/Off Events
    HighBreathingOn = zeros(size(NeuralTime));
    HighBreathingOff = zeros(size(NeuralTime));
    if ~isempty(behavioral_matrix_full) && size(behavioral_matrix_full, 2) >= 8
        breathing_rate_raw = behavioral_matrix_full(:, 8);
        high_breathing_state = breathing_rate_raw > config.breathing_freq_threshold;
%         onsets = find(diff([0; high_breathing_state(:)]) == 1);
%         offsets = find(diff([high_breathing_state(:); 0]) == -1);
        HighBreathingOn(high_breathing_state) = 1;
        HighBreathingOff(~high_breathing_state) = 1;
    end
    
    % High Speed On/Off Events
    HighSpeedOn = zeros(size(NeuralTime));
    HighSpeedOff = zeros(size(NeuralTime));
    high_speed_state = AdjustedXYZ_speed > config.speed_threshold;
%     onsets = find(diff([0; high_speed_state(:)]) == 1);
%     offsets = find(diff([high_speed_state(:); 0]) == -1);
    HighSpeedOn(high_speed_state) = 1;
    HighSpeedOff(~high_speed_state) = 1;

    % Define all event signals and names for the GLM
    event_signals = {IR1ON, IR2ON, HighBreathingOn, HighBreathingOff, HighSpeedOn, HighSpeedOff};
    event_names = {'IR1ON', 'IR2ON', 'HighBreathingOn', 'HighBreathingOff', 'HighSpeedOn', 'HighSpeedOff'};
    ...
    % Loop through events and create predictors
    for ev = 1:length(event_signals)
        ...
        if ~isempty(event_onset_indices)
            event_times = NeuralTime(event_onset_indices);
            event_indicator = histcounts(event_times, time_bins)';
        end
    ...
    %% 2. CONTINUOUS PREDICTORS (None in this model)
    % This section is intentionally left empty as Speed and Breathing are now event-based.
...
%   predictor_info_augmented: Updated predictor info
    ...
    % Original structure: [Bias, Events, Speed, Breathing, Interaction]
    % New structure: [Bias, Events, Speed, Breathing, Interaction, History_lag1, ..., History_lag150]
...
%   feature_importance: Struct with importance for each predictor group
    ...
    breathing_idx = speed_idx + 1;

    feature_groups.Speed = speed_idx;
    feature_groups.Breathing = breathing_idx;

    % Interaction term (if present)
    current_idx = breathing_idx + 1;
    if config.include_interactions
        feature_groups.Speed_x_Breathing = current_idx;
