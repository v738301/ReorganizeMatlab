%% ========================================================================
%  SPIKE-TRIGGERED LFP AVERAGE ANALYSIS
%  Tests for phase-locking vs amplitude modulation
%  ========================================================================
%
%  Purpose: Understand why high SFC but low PPC at 8 Hz
%
%  Method: Compute spike-triggered LFP average for each frequency band
%  - High PPC → sharp, consistent waveform shape
%  - Low PPC → flat or noisy average (no consistent phase)
%
%  Output: Spike-triggered averages for 0.1-20 Hz (1 Hz bins)
%
%% ========================================================================
...
fprintf('=== SPIKE-TRIGGERED LFP AVERAGE ANALYSIS ===\n\n');
...
% LFP filtering - broadband only (for preprocessing)
config.bp_range = [0.1 300];  % Broadband LFP
...
fprintf('  Using BROADBAND LFP (0.1-300 Hz)\n');
...
for sess_idx = 1:num_aversive_sessions
    ...
    session_results = compute_STA_session(NeuralTime, LFP, valid_spikes, period_boundaries, ...
    ...
    save_filename = fullfile(RewardAversivePath, sprintf('%s_sta.mat', base_filename));
...
% Get time boundaries from aversive
...
for sess_idx = 1:num_reward_sessions
    ...
    period_boundaries = [TriggerMid(1), avg_time_boundaries + TriggerMid(1), TriggerMid(end)];
    n_periods = 4;
    n_units = length(valid_spikes);
    ...
    save_filename = fullfile(RewardSeekingPath, sprintf('%s_sta.mat', base_filename));
    save(save_filename, 'session_results', 'config', '-v7.3');
...
fprintf('\n========================================\n');
fprintf('SPIKE-TRIGGERED AVERAGE ANALYSIS COMPLETE!\n');
fprintf('========================================\n');
...
function session_results = compute_STA_session(NeuralTime, LFP, valid_spikes, ...
                                               period_boundaries, n_periods, n_units, Fs, config)
% Compute spike-triggered averages using BROADBAND LFP
    ...
    fprintf('  Computing STAs using broadband LFP...\n');

    % Process each unit (using BROADBAND LFP - no frequency filtering)
    for unit_idx = 1:n_units
        ...
        for period_idx = 1:n_periods
            ...
            spike_indices = spike_indices(spike_indices > window_samples & ...
                                          spike_indices <= length(LFP) - window_samples);

            % Compute STA from BROADBAND LFP
            sta_matrix = zeros(length(spike_indices), 2*window_samples + 1);
            for s = 1:length(spike_indices)
                idx = spike_indices(s);
                sta_matrix(s, :) = LFP(idx - window_samples : idx + window_samples);
            end
