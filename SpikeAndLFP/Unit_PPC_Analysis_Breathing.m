%% ========================================================================
%  UNIT PPC ANALYSIS WITH BREATHING SIGNAL: Period × SessionType × FrequencyBand
%  Pairwise Phase Consistency (PPC) for spike-breathing coupling
%  ========================================================================
%
...
fprintf('=== UNIT PPC ANALYSIS: SPIKE-BREATHING COUPLING ===\n');
fprintf('Pairwise Phase Consistency for Spike-Breathing Coupling\n');
...
for sess_idx = 1:num_aversive_sessions
    ...
    clear filtered_breathing Signal
    ...
    fprintf('  Breathing channel: %d, Units: %d, Periods: %d, Fs: %.0f Hz\n', config.breathing_channel, n_units, n_periods, Fs);
    ...
    for band_idx = 1:n_bands
        ...
        Breathing_filtered = filter_LFP_robust(Breathing, band_range, Fs);
        analytic_signal = hilbert(Breathing_filtered);
        phase_signal = angle(analytic_signal);  % Phase in radians [-π, π]

        % Free up memory
        clear Breathing_filtered analytic_signal;
    ...
    save_filename = fullfile(RewardAversivePath, sprintf('%s_unit_ppc_breathing.mat', base_filename));
...
for sess_idx = 1:num_reward_sessions
    ...
    % Save session results
    [~, base_filename, ~] = fileparts(spike_filename);
    save_filename = fullfile(RewardSeekingPath, sprintf('%s_unit_ppc_breathing.mat', base_filename));
    save(save_filename, 'session_results', 'config', '-v7.3');
