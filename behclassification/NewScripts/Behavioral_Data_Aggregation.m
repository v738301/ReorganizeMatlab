...
% Select aversive session files
[allfiles_aversive, ~, ~, ~] = selectFilesWithAnimalIDFiltering(spike_folder, 999, '2025*RewardAversive*.mat');
fprintf('✓ Found %d aversive spike files\n', length(allfiles_aversive));

% Load aversive predictions matched to spike files
try
...
% Select reward session files
[allfiles_reward, ~, ~, ~] = selectFilesWithAnimalIDFiltering(spike_folder, 999, '2025*RewardSeeking*.mat');
fprintf('✓ Found %d reward spike files\n', length(allfiles_reward));

% Load reward predictions matched to spike files
try
...
fprintf('\nLoading behavioral matrices from spike files...\n');

% Load sorting parameters
[T_sorted] = loadSortingParameters();

% Calculate aversive behavioral matrices from spike files
try
...
% Calculate reward behavioral matrices from spike files
try
