%% ========================================================================
%  BEHAVIORAL PATCH DURATION ANALYSIS: Period × Behavior × ResponseType
...
%  ========================================================================
%
%  Analysis: Duration ~ Period × Behavior × ResponseType
...
fprintf('=== BEHAVIORAL PATCH DURATION ANALYSIS: RESPONSIVE vs NON-RESPONSIVE ===\n');
...
%% ========================================================================
%  SECTION 3: EXTRACT RESPONSIVE AVERSIVE PATCH DURATIONS (Group 1, P1-P4)
%  ========================================================================

fprintf('Extracting RESPONSIVE aversive patch durations (Group 1, P1-P4)...\n');
...
responsive_data.duration = [];

n_valid_responsive = 0;
total_patches_responsive = 0;
clustering_idx = 0;
...
for sess_idx = 1:length(sessions_aversive)
    ...
    for period = 1:4
        ...
        for patch_idx = 1:(length(behavior_changes) - 1)
            ...
            responsive_data.behavior(end+1) = patch_behavior;
            responsive_data.duration(end+1) = patch_duration_seconds;

            total_patches_responsive = total_patches_responsive + 1;
        end
    end
end

fprintf('✓ Processed %d RESPONSIVE aversive sessions\n', n_valid_responsive);
fprintf('  Total patches: %d\n', total_patches_responsive);
fprintf('  Mean patches per session: %.1f\n\n', total_patches_responsive / max(n_valid_responsive, 1));

%% ========================================================================
%  SECTION 4: EXTRACT NON-RESPONSIVE AVERSIVE PATCH DURATIONS (Group 2, P1-P4)
%  ========================================================================

fprintf('Extracting NON-RESPONSIVE aversive patch durations (Group 2, P1-P4)...\n');
...
nonresponsive_data.duration = [];

n_valid_nonresponsive = 0;
total_patches_nonresponsive = 0;
clustering_idx = 0;
...
for sess_idx = 1:length(sessions_aversive)
    ...
    for period = 1:4
        ...
        for patch_idx = 1:(length(behavior_changes) - 1)
            ...
            nonresponsive_data.behavior(end+1) = patch_behavior;
            nonresponsive_data.duration(end+1) = patch_duration_seconds;

            total_patches_nonresponsive = total_patches_nonresponsive + 1;
        end
    end
end

fprintf('✓ Processed %d NON-RESPONSIVE aversive sessions\n', n_valid_nonresponsive);
fprintf('  Total patches: %d\n', total_patches_nonresponsive);
fprintf('  Mean patches per session: %.1f\n\n', total_patches_nonresponsive / max(n_valid_nonresponsive, 1));

%% ========================================================================
%  SECTION 5: EXTRACT REWARD PATCH DURATIONS (P1-P4)
%  ========================================================================
...
fprintf('  Mean patches per session: %.1f\n\n', total_patches_reward / max(n_valid_reward, 1));
...
merged_nonresponsive.duration = [nonresponsive_data.duration(:); reward_data.duration(:)];
...
combined_data.duration = [responsive_data.duration(:); merged_nonresponsive.duration(:)];
...
tbl = table(combined_data.session_id, ...
            ...
            'VariableNames', {'Session', 'Period', 'Behavior', 'Duration', 'ResponseType'});
...
fprintf('    - Responsive: %d sessions, %d patches\n', n_valid_responsive, total_patches_responsive);
fprintf('    - Non-Responsive: %d sessions (%d aversive + %d reward), %d patches\n', ...
        n_valid_nonresponsive + n_valid_reward, n_valid_nonresponsive, n_valid_reward, ...
        total_patches_nonresponsive + total_patches_reward);
...
fprintf('Formula: Duration ~ Period * Behavior * ResponseType + (1|Session)\n');
fprintf('This may take a few minutes...\n\n');

try
    lme_full = fitlme(tbl, 'Duration ~ Period * Behavior * ResponseType + (1|Session)', ...
                      'FitMethod', 'REML');
...
for b = 1:config.n_behaviors
    ...
    for p = 1:4
        ...
        % Print results
        fprintf('  P%d: Resp=%.2fs, NonResp=%.2fs, Diff=%.2fs (SE=%.2f), t=%.2f, p=%.4f', ...
...
%% ========================================================================
%  SECTION 9: VISUALIZE - Session-level mean durations
%  ========================================================================
...
for b = 1:config.n_behaviors
    ...
    for p = 1:4
        % Responsive mean
        responsive_period_data = responsive_beh.mean_Duration(double(responsive_beh.Period) == p);
        ...
        % Non-Responsive mean
        nonresponsive_period_data = nonresponsive_beh.mean_Duration(double(nonresponsive_beh.Period) == p);
    ...
    all_vals = [mean_responsive; mean_nonresponsive; ...
                responsive_beh.mean_Duration; nonresponsive_beh.mean_Duration];
    y_min = min(all_vals);
    y_max = max(all_vals);
    y_range = y_max - y_min;
...
% Add overall title
sgtitle({'Behavioral Patch Duration: Responsive vs Non-Responsive', ...
...
if ~isempty(pred_duration)
    ...
    % Add overall title
    sgtitle({'Model Predictions: Behavioral Patch Duration (Responsive vs Non-Responsive)', ...
...
% results.total_patches_responsive = total_patches_responsive;
% results.total_patches_nonresponsive = total_patches_nonresponsive;
% results.total_patches_reward = total_patches_reward;
%
% save('duration_responsive_vs_nonresponsive_results.mat', 'results');
%
% fprintf('✓ Results saved to: duration_responsive_vs_nonresponsive_results.mat\n');
fprintf('\n=== ANALYSIS COMPLETE ===\n');
